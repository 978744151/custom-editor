    ...
    <el-container>
      ...
      <el-aside :width="leftWidth">
        <div class="fields-list">
          <template v-if="customFields && customFields.length > 0">
            ...
            <template v-else>
              <!-- <el-link class="field-title" :underline="false"
              ...
              <draggable tag="ul" :list="customFields" :group="{ name: 'form', pull: 'clone', put: false }"
                ghost-class="ghost" :sort="false">
                <template v-for="(item, index) in customFields">
                  <el-tooltip v-if="item.tips" effect="dark" :content="item.tips" :key="index">
                    <li class="field-label" :key="index">
                      <img :src="item.icon" alt="" class="field-label-image">
                      <a @click="handleFieldClick(item)">
                        <i class="icon iconfont" :class="item.icon"></i>
                        <span>{{ item.title || item.label }}</span>
                      </a>
        ...
        <el-header class="widget-container-header">
          <div>
            <template v-if="undoRedo">
              <el-button type="text" size="medium" icon="el-icon-refresh-left" :disabled="historySteps.index == 0"
          ...
          <div style="display: flex; align-items: center;">
            ...
            <el-button v-if="toolbar.includes('avue-doc')" type="text" size="medium" icon="el-icon-document"
              @click="handleAvueDoc">Avue文档</el-button> -->
            <!-- <el-button v-if="toolbar.includes('import')" type="text" size="medium" icon="el-icon-upload2"
              @click="importJsonVisible = true">导入JSON</el-button> -->
            <el-button v-if="toolbar.includes('generate')" type="text" size="medium" icon="el-icon-download"
              @click="handleGenerateJson">生成JSON</el-button>
            <el-button v-if="toolbar.includes('preview')" type="text" size="medium" icon="el-icon-view"
              @click="handlePreview">预览</el-button>
            <el-button v-if="toolbar.includes('clear')" class="danger" type="text" size="medium" icon="el-icon-delete"
      ...
      <el-aside class="widget-config-container" :width="rightWidth">
        <el-tabs v-model="configTab" stretch>
          <el-tab-pane label="页面设置" name="page" style="padding: 0 10px;">
            <base-config :data="widgetForm"></base-config>
          </el-tab-pane>

          <el-tab-pane label="组件设置" name="widget" style="padding: 0 10px;">
            <widget-config :data="widgetFormSelect" :default-values="defaultValues"></widget-config>
          </el-tab-pane>
      ...
      <!-- 弹窗 -->
      <!-- 导入JSON -->
      <el-drawer title="导入JSON" :visible.sync="importJsonVisible" size="50%" append-to-body destroy-on-close>
        <monaco-editor v-model="importJson" keyIndex="import" height="82%"></monaco-editor>
      ...
      <!-- 生成JSON -->
      <el-drawer title="生成JSON" :visible.sync="generateJsonVisible" size="50%" append-to-body destroy-on-close>
        <monaco-editor v-model="option" keyIndex="generate" height="82%" :read-only="true"></monaco-editor>
        <div class="afd-drawer-foot">
          <el-button size="medium" type="primary" @click="handleGenerate">生成</el-button>

          <el-popover placement="top" trigger="hover" width="350px">
            <el-form v-model="configOption" style="padding: 0 20px" label-suffix="：" label-width="180px"
              label-position="left">
              ...
              <el-form-item label="缩进长度-空格数量">
                <el-slider v-model="configOption.space" show-stops :marks="{ 1: '1', 2: '2', 3: '3', 4: '4' }" :min="1"
                  :max="4" :step="1"></el-slider>
              </el-form-item>
              <el-form-item label="引号类型">
                <el-switch v-model="configOption.quoteType" active-value="single" inactive-value="double"
                  active-text="单引号" inactive-text="双引号"></el-switch>
              </el-form-item>
            ...
            <el-button size="medium" type="primary" @click="handleCopy" slot="reference"
              style="margin-left: 10px;">复制</el-button>
          </el-popover>
        </div>
      </el-drawer>
      <!-- 预览 -->
      <el-drawer title="预览" :with-header="false" :visible.sync="previewVisible" size="100%" append-to-body
        :before-close="handleBeforeClose">
        <avue-form v-if="previewVisible" ref="form" :style="{ backgroundColor: widgetForm.background }"
          class="afd-preview-form" :option="option" v-model="form" @submit="handlePreviewSubmit"></avue-form>
        <div class="afd-drawer-foot">
...
export default {
  name: "FormDesign",
  components: { Draggable, MonacoEditor, WidgetForm, WidgetConfig, FormConfig, baseConfig },
  mixins: [history],
  props: {
    options: {
      type: [Object, String],
  ...
  watch: {
    ...
    widgetFormSelect: {
      handler() {
        console.log('widgetFormSelect', this.widgetFormSelect, this.widgetForm)
  ...
  methods: {
    ...
    this.option = this.widgetForm
        // this.transformToAvueOptions(this.widgetForm, true).then(data => {
        //   this.option = { ...data, submitBtn: false, emptyBtn: false }
