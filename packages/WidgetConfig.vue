<template>
  <div class="widget-config">
    <el-form label-suffix="：" v-if="this.data && Object.keys(this.data).length > 0" labelPosition="left"
      labelWidth="90px" size="small">
      <component :is="getComponent" :data="data" :default-values="defaultValues"></component>

      <el-collapse v-model="collapse" v-if="false">
        <el-collapse-item name="1" title="基本属性">
          <el-form-item label="类型" v-if="data.type && !data.component">
            <el-select v-model="data.type" style="width:100%;" placeholder="请选择类型" @change="handleChangeType">
              <el-option-group v-for="group in fields" :key="group.title" :label="group.title">
                <el-option v-for="item in group.list" :key="item.type" :label="item.label" :value="item.type">
                </el-option>
              </el-option-group>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="标题宽度" v-if="!['group', 'dynamic', 'title'].includes(data.type)">
            <el-input-number v-model="data.labelWidth" :min="80" :step="10" controls-position="right" placeholder="标签宽度"
          ...
          <el-form-item label="数据类型"
            ...
            <template slot="label">
              <el-link :underline="false" href="https://avuejs.com/docs/datatype.html" target="_blank">数据类型 <i
          ...
          <el-form-item label="字段提示">
            <template slot="label">
              <el-link :underline="false" href="https://avuejs.com/form/form-tip.html" target="_blank">字段提示 <i
                  class="el-icon-question"></i></el-link>
            </template>
            <el-input v-model="data.tip" clearable placeholder="字段提示"></el-input>
          </el-form-item>
          <el-form-item v-if="data.tip && !['upload'].includes(data.type)" label="字段提示位置" label-width="110px">
          ...
          <el-form-item label="标题提示">
            <template slot="label">
              <el-link :underline="false" href="https://avuejs.com/form/form-tip.html" target="_blank">标题提示 <i
                  class="el-icon-question"></i></el-link>
            </template>
            <el-input v-model="data.labelTip" clearable placeholder="标题提示"></el-input>
          </el-form-item>
          <el-form-item v-if="data.labelTip && !['upload'].includes(data.type)" label="标题提示位置" label-width="110px">
          ...
          <el-form-item v-if="!['color', 'dynamic', 'group', 'ueditor', 'upload', 'map'].includes(data.type)"
            ...
            <template v-if="defaultValues && defaultValues[data.type]">
              <el-select v-model="data.value" allow-create clearable filterable placeholder="默认值">
                <el-option v-for="item in defaultValues[data.type]" :key="item.value" :label="item.label"
                  :value="item.value">
                </el-option>
              </el-select>
            </template>
    ...
    <template v-else>
      <avue-empty v-if="avueVersion('2.9.4')" desc="拖拽字段进行配置" style="margin-top: 100%;"></avue-empty>
      <el-empty v-else description="拖拽字段进行配置" style="margin-top: 100%;"></el-empty>
    </template>
  </div>
</template>
